...
public class MediaMuxerThread extends Thread {
    ...
    /**
     * @param filePath String filePath = Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + "myav.mp4";
     */
    // 开始音视频混合任务
    ...
    private void readyStart() throws IOException {
        String recordVideoPath = RecordVideoManage.getInstance().getRecordVideoPath();
        ...
        muxerDatas.clear();
//初始化
        mediaMuxer = new MediaMuxer(recordVideoPath, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);
        if (audioThread != null) {
        ...
        Log.e(TAG, "readyStart(String filePath, boolean restart) 保存至:" + recordVideoPath);
    }
